  ...
  const handleImportExcel = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (evt) => {
      try {
        ...
        if (data.length === 0) {
          setStatusMsg("File kosong atau format salah.");
          return;
        }

        // Group rows by Journal No
        const journalMap = new Map<string, Journal>();

        data.forEach(row => {
          // Find common header variations
          ...
          const no = String(getVal(["nomor", "no", "bukti"]));
          const date = String(getVal(["tanggal", "date"]));
          const desc = String(getVal(["keterangan", "desc"]));
          const accCode = String(getVal(["akun", "account", "code"]));
          const debit = parseFloat(getVal(["debit", "db"])) || 0;
          const credit = parseFloat(getVal(["kredit", "cr", "credit"])) || 0;

          if (!no) return;

          if (!journalMap.has(no)) {
            let parsedDate = new Date().toISOString().split('T')[0];
            // Handle excel date serials or strings
            if (typeof date === 'number') {
               parsedDate = new Date(Math.round((date - 25569) * 86400 * 1000)).toISOString().split('T')[0];
          ...
          if (accCode) {
            journalMap.get(no)!.details.push({
              ...
              accountId: accCode,
              accountName: "", // We can try to look it up but it's okay
              debit,
              credit
            });
          }
        ...
        const newJournalsList = Array.from(journalMap.values());
        
        if (newJournalsList.length === 0) {
          setStatusMsg("Tidak ada data jurnal valid yang ditemukan.");
          return;
        }

        const merged = [...newJournalsList, ...journals];
        merged.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
        
        setJournals(merged);
        await saveJournals(merged);
        
        setStatusMsg(`Berhasil import ${newJournalsList.length} jurnal ✅`);
        setTimeout(() => setStatusMsg(null), 3000);
      } catch (err) {
        console.error(err);
                ...
                <div className="space-y-5">
                  <div>
                    ...
                    <DatePicker 
                      ...
                      onChange={(date: Date | null) => date && setNewJournal({...newJournal, date: format(date, 'yyyy-MM-dd')})}
                      dateFormat="dd/MM/yyyy"
